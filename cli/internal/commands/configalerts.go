				...
				Rules: []alertRule{
					{
						...
						Annotations: map[string]string{
							"summary": fmt.Sprintf("Claude Code cost increased by more than $%s in the last hour.", strconv.FormatFloat(costThreshold, 'f', -1, 64)),
						},
					},
					{
						...
						Labels:       map[string]string{"severity": "warning"},
						Annotations: map[string]string{
							"summary": fmt.Sprintf("Claude Code hit more than %d API rate-limit (429) errors in the last 5 minutes.", rateLimitThreshold),
						},
					},
					{
						UID:       "agentobs-tool-failure-spike",
						Title:     "Claude Code tool call failures",
						Condition: "C",
						Data: []queryData{
							{
								RefID:             "A",
								...
								Model: map[string]interface{}{
									"refId":  "A",
									"rawSql": "SELECT count() AS value FROM otel.otel_logs WHERE LogAttributes['event.name']='tool_result' AND LogAttributes['success']='false'",
									"format": 1,
								},
							},
							{
						...
						Labels:       map[string]string{"severity": "warning"},
						Annotations: map[string]string{
							"summary": fmt.Sprintf("More than %d tool calls failed in the last 15 minutes.", toolFailureThreshold),
						},
					},
				},
			},
...
func buildContactPoints(contactType, webhookURL string) contactPointsFile {
				...
				Receivers: []receiver{
					{
						...
						Type:     contactType,
						Settings: map[string]string{"url": webhookURL},
					},
				},
			},
		},
		Policies: []policy{
			{OrgID: 1, Receiver: "agentobs-webhook"},
		},
	}
}
...
func ConfigAlertsCmd() *cobra.Command {
	var composeFile, webhookURL, contactType string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contactOut, err := yaml.Marshal(buildContactPoints(resolvedContactType, resolvedWebhookURL))
			if err != nil {
				return err
			}
