...
Optional, opt-in, and off by default. Config is per-project: `Credentials.Projects` is a list, each entry (`Project`) carrying its own `Path`, `SpaceID`, `Watermark`, and `RecentTurns` dedup set, because a shared watermark would let one project's sync skip another's turns. `SyncAll` (`sync.go`) iterates the entries and calls `SyncProject` on each independently, recording a failure against that project's `Result.SyncErr` and moving on rather than aborting the run -- it returns a non-nil error only when every project failed, so one broken space doesn't stop the others from syncing. A config from before this (one flat `space_id`/`watermark`) is detected by its `version` field, backed up to `memory.json.bak`, and migrated (`migrateV1`) into one entry per project path, each seeded from the old shared space/watermark.

`SyncProject` gathers turns from two readers per project — `ClaudeCodeSource` parses `~/.claude/projects/*/*.jsonl` for paired prompt+response text, `ClickHouse.PromptOnlyTurns` queries prompt-only turns for every non-Claude-Code ServiceName plus `ClickHouse.SessionStats` for per-session cost — then filters by that project's deny-by-default `AllowsPath`, masks with `MaskText`, and writes them in chunks of 100 to `POST /v1/record/batch`. Gemini CLI contributes nothing: its telemetry carries no directory attribute, so nothing from it can ever pass the allowlist, and the `otel_logs` branch that once queried it is gone. Of the hook-shim agents, only **Cursor** is confirmed to actually deliver turns here — it stamps `workspace_roots` on its hook payloads natively, which is what the allowlist filters on. Copilot, Codex, and OpenCode are not confirmed: Copilot/Codex have never been verified to send `workspace_roots` at all; OpenCode's plugin now sends it (via `PluginInput.directory`) and a separate bug that silently dropped its payload before delivery (calling `.stdin()` on Bun's `$` result, which Bun's real shipped typings declare as a non-callable property — fixed by redirecting stdin from a `Response` instead) has been fixed and verified against a real Bun install in isolation, but the plugin has never been loaded by an actual OpenCode process end-to-end.

For Claude Code, a turn's `Response` is every text block the assistant produced for that prompt, not just the first: `ClaudeCodeSource` closes a turn on the next real user prompt (or EOF), so a preamble before a tool call and the answer after it both land in the same turn, joined with `"\n"`; `InputTokens`/`OutputTokens` sum every assistant message in that span. `TurnID`/`Timestamp`/`Model` come from the last text-bearing assistant message, so they always trace back to the same transcript line.

Claude Code's conversation text never enters ClickHouse -- its transcript is read at sync time and goes straight to the API. The hook-shim agents' prompt text is the exception: it's read back out of `otel_traces` (`SpanAttributes['gen_ai.prompt.0.content']`), where it already lands as part of normal span export, keyed to a working directory via the `workspace_roots` attribute -- which `addCommonAttributes` (`cursorhook/hook.go`) stamps only when the incoming hook payload actually contains that key, not on every span. Sources are queried from `watermark - 24h`, not the raw watermark, so a turn that lands after a faster source already advanced the watermark past it still gets picked up and deduped via `Credentials.RecentTurns` instead of silently lost -- but only on the stored-watermark path; an explicit `--since` is a user-named backfill window and is used as-is, with no lookback added, ignoring the stored watermark entirely, so it can't silently over-fetch. A failed write still advances the watermark over whatever contiguous prefix `RecordBatch` reports as accepted before the error, so a partial multi-chunk failure doesn't re-push chunks the server already took; only the unaccepted remainder retries next run. A ClickHouse outage degrades the run rather than ending it. `--quiet` (the flag the wizard's cron line uses) only suppresses the routine success line and the two routine, expected-every-run skip counts (already-synced, outside-allowlist); genuine degradations (ClickHouse unreachable, unreadable files, unparseable rows) always print to stderr regardless of `--quiet`.

**The Stop hook** (`commands/memory.go`'s `memoryHookCmd`, registered by `agents.RegisterStopHook` into `~/.claude/settings.json` as `agentobs memory hook`) is `agentobs`'s entry point for Claude Code's Stop event. It always returns nil/exits 0 and never writes to stdout or stderr -- a Stop hook's output is read by Claude Code, so a broken connector must stay invisible to it; every failure path instead calls `memory.LogHook`, which appends to `~/.config/agentobs/sync.log` (capped at 1 MiB, oldest half dropped). The first invocation (no `--detached`) re-execs itself with `--detached` and the payload written to a temp file, then calls `Release()` and returns at once without waiting -- a sync is network I/O against AnonaMemory and must not delay session end. Only the detached child does the real work, under a per-project `flock` (`ProjectLock`) so two sessions ending at once can't double-push the same project.
...
- **Privacy defaults differ by agent, and `mask_prompts` is the exception, not the rule.** Claude Code's `log_user_prompts`/`log_tool_details` and Gemini CLI's `log_prompts` (`builtin.yaml`) all default `false`, so prompt/tool-detail capture is opt-**in** for those two agents. `mask_prompts`, asked once per `connect` run for the four hook-shim agents (Cursor, Copilot, Codex, OpenCode), also defaults `false` (`boolPtr(false)` at both call sites in `connect.go`) -- but `false` there means *not* masked: `cursorhook/event_attributes.go`'s `addEventSpecificAttributes` stores the full prompt in `gen_ai.prompt.0.content` whenever `cfg.MaskPrompts` is false. So prompt capture is opt-**out** for those four; answering yes to the mask-prompts question is what makes it opt-in.
